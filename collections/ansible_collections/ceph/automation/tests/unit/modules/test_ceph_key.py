...
from ansible_collections.ceph.automation.plugins.modules import ceph_key, ceph_key_info
...
class TestCephKeyModule(object):
    ...
    @mock.patch('ansible_collections.ceph.automation.plugins.modules.ceph_key_info.exec_commands')
    @pytest.mark.parametrize('output_format', ['json', 'plain', 'xml', 'yaml'])
    def test_state_info(self, m_exec_commands, m_exit_json, output_format):
        ca_test_common.set_module_args({"cluster": "ceph",
                                        "name": "client.admin",
    ...
    @mock.patch('ansible_collections.ceph.automation.plugins.modules.ceph_key.generate_secret')
    @mock.patch('ansible.module_utils.basic.AnsibleModule.exit_json')
